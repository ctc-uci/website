export interface Project {
  name: string;
  startYear: string;
  endYear: string;
  description: string;
  image: string;
}

export const PROJECT_DATA: Project[] = [
  {
    name: "The Hub OC",
    startYear: "2026",
    endYear: "2027",
    description:
      "The Hub OC is a family of programs that practice “contagious generosity” to support Orange County families and individuals. Through an umbrella network of youth centers, resource hubs, community spaces, and a food pantry, they connect neighbors with immediate practical help and relational support to encourage people to thrive.",
    image: "the-hub-oc.png",
  },
  {
    name: "Wells of Life",
    startYear: "2026",
    endYear: "2027",
    description:
      "Wells of Life is a nonprofit dedicated to ending the water crisis by providing rural Ugandans with access to safe, clean water. They drill new borehole wells, restore abandoned ones, and run Healthy Village (WASH) programs that teach sanitation and hygiene to help communities thrive long-term.",
    image: "wells-of-life.png",
  },
  {
    name: "UPLIFT Youth Foundation",
    startYear: "2026",
    endYear: "2027",
    description:
      "UPLIFT Youth Foundation is a nonprofit dedicated to supporting youth mental wellness. They partner with schools and communities to offer yoga, mindfulness series, meditation, journaling, and retreats that help teens build meaningful connection, reduce stress, and develop practical tools for navigating everyday challenges.",
    image: "uplift-youth-foundation.png",
  },
];

export const PREVIOUS_DATA: Project[] = [
  {
    name: "Celebrating Life Community Health Center",
    startYear: "2025",
    endYear: "2026",
    description:
      "Celebrating Life Community Health Center is dedicated to delivering high-quality medical care and social services to Orange County’s most vulnerable populations.",
    image: "celebrating-life-community-health-center.svg",
  },
  {
    name: "Global Creation Foundation",
    startYear: "2025",
    endYear: "2026",
    description:
      "Global Creation Foundation's mission is “Arts Education For Every Nation.” They partner with schools, churches, and orphanages to launch sustainable programs using donated instruments and video-based lessons, helping students heal, grow, and express themselves through creativity.",
    image: "global-creation-foundation.png",
  },
  {
    name: "Elder Law & Disability Rights Center",
    startYear: "2025",
    endYear: "2026",
    description:
      "Elder Law & Disability Rights Center works to ensure dignity and justice for elderly individuals, people with disabilities, and those experiencing homelessness.",
    image: "elder-law-and-disability-rights-center-modified.png",
  },
  {
    name: "Colette's Childrens Home",
    startYear: "2024",
    endYear: "2025",
    description:
      "Colette’s Children’s Home provides emergency housing for homeless women/children and assist in maintaining employment. They aim to provide the resources needed to achieve self-sufficiency.",
    image: "colettes-childrens-home.png",
  },
  {
    name: "Center Stage",
    startYear: "2024",
    endYear: "2025",
    description:
      "Center Stage’s mission is to elevate the art of Chinese dance through passionate teaching, immersive performances, and cultural exchange, to nurture appreciation for Chinese heritage.",
    image: "center-stage.png",
  },
  {
    name: "La Peña",
    startYear: "2024",
    endYear: "2025",
    description:
      "La Pena is a central site for Latinx, Caribbean, and Indigenous diasporic communities of the Bay Area, preserving cultural traditions and nurturing grassroots social justice movements",

    image: "la-pena.png",
  },
  {
    name: "Achievement Institute for STEM Scholars",
    startYear: "2023",
    endYear: "2024",
    description:
      "A college readiness program catered towards high-achieving, economically disadvantaged high school students interested in pursuing a STEM career.",
    image: "Achievement-Institute-Of-STEM-Scholars.png",
  },
  {
    name: "Stand Up To Trash",
    startYear: "2023",
    endYear: "2024",
    description:
      "Stand Up to Trash is an org that protects ocean ecosystems by raising awareness of the negative effects caused by plastic pollution through environmental education for future generations.",
    image: "Stand-Up-To-Trash.png",
  },
  {
    name: "Feeding Pets of the Homeless",
    startYear: "2023",
    endYear: "2024",
    description:
      "Feeding Pets of the Homeless is dedicated to providing basic emergency veterinary care and food for the pets for those suffering from homelessness.",
    image: "Feeding-Pets-Of-The-Homeless.png",
  },
  {
    name: "Get Inspired",
    startYear: "2022",
    endYear: "2023",
    description:
      "We built a new database system for them to better organize and report their Pismo Clam restoration data!",
    image: "Get-Inspired.png",
  },
  {
    name: "Cell Dogs",
    startYear: "2022",
    endYear: "2023",
    description:
      "We created a canine tracker for them to manage their trainee dogs in correctional programs and adoption!",
    image: "Cell-Dogs.png",
  },
  {
    name: "Patriots and Paws",
    startYear: "2022",
    endYear: "2023",
    description:
      "We designed a system for them to manage and schedule pickups for their furniture donations for veterans!",
    image: "Patriots-and-Paws.png",
  },
  {
    name: "OC Habitats",
    startYear: "2021",
    endYear: "2022",
    description:
      "We constructed digital forms for their habitat overseers to provide them with better data on their conservation efforts!",
    image: "OC-Habitats.png",
  },
  {
    name: "The Literacy Project",
    startYear: "2021",
    endYear: "2022",
    description:
      "We created a platform to manage literacy scorecards for children enrolled in their award-winning reading program!",
    image: "The-Literacy-Project.png",
  },
  {
    name: "Find Your Anchor",
    startYear: "2021",
    endYear: "2022",
    description:
      "We made a map for them to track where their mental health anchor boxes are traveling throughout the world!",
    image: "Find-Your-Anchor.png",
  },
  {
    name: "Abound Food Care",
    startYear: "2021",
    endYear: "2022",
    description:
      "We created a digital system for them to better connect with their volunteers and engage them in events!",
    image: "Abound-Food-Care.png",
  },
  {
    name: "Justice for Families",
    startYear: "2020",
    endYear: "2021",
    description:
      "We created a mobile application for the families of incarcerated loved ones to find community and support!",
    image: "Justice-For-Families.png",
  },
  {
    name: "Waymakers",
    startYear: "2020",
    endYear: "2021",
    description:
      "We created a platform to electronically capture volunteer and inventory data about their different programs!",
    image: "Waymakers.png",
  },
  {
    name: "Child Creativity Lab",
    startYear: "2020",
    endYear: "2021",
    description:
      "We created an online waiver system and a platform to sell educational kits with instructional videos for children!",
    image: "Child-Creativity-Lab.png",
  },
  {
    name: "Crime Survivors",
    startYear: "2020",
    endYear: "2020",
    description:
      "We created a website to replace their paper pamphlets with resources and information for crime victims!",
    image: "Crime-Survivors.png",
  },
];
